...
export function Features() {
  return (
    <section className="px-5 py-8">
      ...
      <h3 className="mb-5 text-[22px] font-[800] tracking-[-0.01em] text-slate-950">選ばれる、3つの理由。</h3>
      <div className="flex flex-col gap-3">
        {CONTENT.reasons.map((r) => (
          <div key={r.num} className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mb-2 text-base font-[800] tracking-[-0.01em] text-slate-950">{r.title}</div>
            <div className="text-[13px] leading-[1.7] text-slate-500">{r.body}</div>
          </div>
        ))}
      </div>
    </section>
